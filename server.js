...
function calcSignal(bars,livePrice,cfg){
  ...
  if(!emaOk){signal="EMA ÜSTÜNDE";signalColor="#F97316";}
  if(rOn&&rsi>70){signal="RSI AŞIRI ALIM";signalColor="#EF4444";}

  // Proximity alerts — is price near any entry?
  const alerts=[];
  entries.forEach(e=>{
  ...
  return <div style={{background:hot?"rgba(239,68,68,0.06)":P.card,border:`1px solid ${hot?"rgba(239,68,68,0.3)":P.border}`,borderRadius:14,padding:14,marginBottom:12,animation:hot?"pulse 1.5s infinite":"fadeIn 0.3s"}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
      <div style={{display:"flex",alignItems:"center",gap:8}}>
        ...
        <span style={{fontFamily:Fd,fontSize:18,color:P.white,fontWeight:700}}>📡 Canlı Sinyal</span>
      </div>
      <div style={{display:"flex",alignItems:"center",gap:6}}>
        <span style={{fontFamily:Fm,fontSize:11,color:P.ash}}>{dN(sym)} • {tf}</span>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(130px,1fr))",gap:6,marginBottom:10}}>
      {sig.entries.map(e=>{
        ...
        return <div key={e.layer} style={{background:veryNear?"rgba(45,199,113,0.12)":near?"rgba(45,199,113,0.06)":P.surface,border:`1px solid ${veryNear?P.jade+"66":near?P.jade+"33":P.border}`,borderRadius:10,padding:"8px 10px",animation:veryNear?"pulse 1s infinite":"none"}}>
          ...
          <div style={{fontFamily:Fm,fontSize:10,color:dist>0?P.jade:P.danger}}>{dist>0?"▼":"▲"} {Math.abs(dist).toFixed(2)}%</div>
        </div>;
      })}
      {/* TP Card */}
      <div style={{background:"rgba(123,92,245,0.06)",border:`1px solid rgba(123,92,245,0.3)`,borderRadius:10,padding:"8px 10px"}}>
        <div style={{fontFamily:Fm,fontSize:9,color:P.violet,letterSpacing:"0.1em",fontWeight:600,marginBottom:3}}>🎯 HEDEF</div>
    ...
    {sig.alerts.length>0&&<div style={{background:P.surface,borderRadius:8,padding:"8px 10px"}}>
      {sig.alerts.map((a,i)=><div key={i} style={{display:"flex",alignItems:"center",gap:6,padding:"3px 0",fontFamily:Fm,fontSize:11}}>
        <span style={{color:a.hot?P.danger:P.ember,animation:a.hot?"pulse 0.5s infinite":"none"}}>{a.hot?"🔴":"🟡"}</span>
        ...
        {a.hot&&<span style={{color:P.danger,fontWeight:700,fontSize:10,animation:"pulse 0.8s infinite"}}>⚡ YAKIN!</span>}
      </div>)}
    </div>}
  </div>;
}

// ═══ PAPER TRADING ENGINE v4 — OKX Perpetual Futures Rules ═══
// Flow: coin seç → OKX'ten max lever çek → otomatik seç → %1 risk → kontrat = risk × lever → hacim = kontrat × 2
// Hedge mode: aynı coin'de hem LONG hem SHORT açılabilir
// Backtest (Martingale) ve Paper Trade BAĞIMSIZ — sadece APTR mesafesini paylaşırlar
function createPaperEngine(initialCap=10000){
  return{
    ...
    maxPositions:10,// toplam max açık pozisyon
    ...
    tick(sym,price){
      ...
      // ── 1) Check SL / TP / Liq on existing positions (copy array to avoid mutation issues) ──
      const toClose=[];
      ...
      // ── 2) Position sizing (OKX futures rules) ──
      ...
      const notional=+(margin*maxLev).toFixed(0);// kontrat değeri
      const qty=notional/price;// coin adedi
      const volume=notional*2;// hacim = kontrat × 2
      // SL distance: loss at SL = riskAmt → slDist = riskAmt / qty = price / maxLev
      ...
      // ── 3) LONG entry — Martingale katman aralığı backtestten gelir (APTR) ──
      const fA=b.cfg.mA||80;// APTR mesafesi — backtestle paylaşılan tek parametre
      ...
      // ── 4) SHORT entry — HEDGE MODE (bağımsız, aynı anda long+short) ──
...
// ═══ PAPER TRADING PANEL v4 — OKX Auto Leverage, %1 Risk, Hedge Mode ═══
function PaperPanel({engine,setEngine,syms,cfg}){
  ...
  const start=()=>{
    ...
    setLogs([{t:Date.now(),msg:`🚀 Futures Paper — $10K — Risk %1 — Hedge Mode ON — ${selected.length} coin`},{t:Date.now()-1,msg:`⚡ Kaldıraçlar: ${levList}`}]);
          ...
          try{
            ...
            if(evts){
              setLogs(prev=>[...evts.map(ev=>{
                ...
                if(ev.type==="TP")return{t:Date.now(),msg:`✅ TP ${ev.side} ${cn} | ${ev.lever}x — $${ev.price} — +$${ev.pnl}`};
                if(ev.type==="SL")return{t:Date.now(),msg:`⛔ SL ${ev.side} ${cn} | ${ev.lever}x — $${ev.price} — $${ev.pnl}`};
                if(ev.type==="LIQ")return{t:Date.now(),msg:`💀 LIQ ${ev.side} ${cn} — $${Math.abs(ev.pnl)} margin kaybedildi`};
  ...
  const stop=()=>{
    ...
    Object.keys(wsRefs.current).forEach(k=>{try{wsRefs.current[k].close();}catch(e){}});
    wsRefs.current={};
    setLogs(prev=>[{t:Date.now(),msg:"⏹ Durduruldu"},...prev]);
  };
  ...
  return <div style={{position:"fixed",bottom:0,left:0,right:0,zIndex:998,background:P.card,borderTop:`2px solid ${running?P.jade:P.border}`,boxShadow:"0 -8px 30px rgba(0,0,0,0.4)"}}>
    ...
    <div onClick={()=>setExpanded(!expanded)} style={{padding:"8px 18px",display:"flex",alignItems:"center",justifyContent:"space-between",cursor:"pointer",borderBottom:expanded?`1px solid ${P.border}`:"none"}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:8}}>
        ...
        <span style={{fontFamily:Fm,fontSize:14,color:P.ash,transform:expanded?"rotate(180deg)":"none",transition:"0.2s"}}>▲</span>
      </div>
    </div>

    {expanded&&<div style={{padding:"12px 18px",maxHeight:400,overflowY:"auto"}}>
      {!running?<>
        <div style={{fontFamily:Fm,fontSize:10,color:P.violet,letterSpacing:"0.12em",marginBottom:6,fontWeight:600}}>OKX PERPETUAL FUTURES — PAPER TRADE</div>

        {/* Info Box */}
        <div style={{background:P.surface,borderRadius:8,padding:"8px 12px",marginBottom:10,fontFamily:Fm,fontSize:10,lineHeight:1.6}}>
          <div style={{color:P.mist}}>📋 <span style={{color:P.white,fontWeight:600}}>Kurallar:</span></div>
          <div style={{color:P.ash}}>• Coin seç → OKX'ten max kaldıraç otomatik gelir</div>
          <div style={{color:P.ash}}>• Her işlemde bakiyenin <span style={{color:P.ember,fontWeight:600}}>%1</span>'i riske edilir → Risk = $100</div>
          <div style={{color:P.ash}}>• Kontrat = Risk × Kaldıraç → örn: $100 × 125x = <span style={{color:P.white,fontWeight:600}}>$12,500</span></div>
          <div style={{color:P.ash}}>• Hacim = Kontrat × 2 → <span style={{color:P.cobalt,fontWeight:600}}>$25,000</span></div>
          <div style={{color:P.ash}}>• <span style={{color:P.jade}}>Hedge Mode</span>: Aynı coin'de eş zamanlı Long + Short</div>
          <div style={{color:P.ash}}>• R:R 1:2 • SL ve TP otomatik • Max <span style={{color:P.ember,fontWeight:600}}>10 pozisyon</span></div>
          <div style={{color:P.ash,fontSize:9,marginTop:2}}>⚠️ Backtest (Martingale) ve Paper Trade bağımsız — sadece APTR katman mesafesini paylaşır</div>
        </div>

        {/* Coin Selection with leverage badges */}
        <div style={{fontFamily:Fm,fontSize:9,color:P.ash,marginBottom:4}}>Coin seç {swapInfo?"(OKX max kaldıraç gösterilir)":"(yükleniyor...)"}:</div>
        ...
        <button onClick={start} disabled={!selected.length||loading} style={{fontFamily:Fs,fontSize:12,fontWeight:700,padding:"9px 24px",borderRadius:10,background:selected.length?`linear-gradient(135deg,${P.jade},#1a9d5c)`:"#2A2D3A",border:"none",color:P.white,cursor:selected.length?"pointer":"not-allowed",boxShadow:selected.length?`0 4px 16px ${P.jade}40`:"none"}}>🚀 Başlat — {selected.length} coin — Hedge Mode — %1 Risk</button>
      </>:<>
        {/* Stats */}
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(85px,1fr))",gap:4,marginBottom:8}}>
          {[
            ["BAKİYE","$"+eq.toLocaleString("en"),ret>=0?P.jade:P.danger],
            ["MARJİN","$"+(stats?.marginUsed||0).toLocaleString("en"),P.violet],
        ...
        {/* Open Positions */}
        {engine?.positions?.length>0&&<div style={{marginBottom:6}}>
          <div style={{fontFamily:Fm,fontSize:9,color:P.ember,letterSpacing:"0.1em",marginBottom:3,fontWeight:600}}>AÇIK POZİSYONLAR ({engine.positions.length}) — HEDGE MODE</div>
          <div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse",fontFamily:Fm,fontSize:9}}>
            <thead><tr>{["#","Coin","Yön","Kaldıraç","Giriş","Şimdi","Margin","Kontrat","Hacim","SL","TP","Liq","K/Z","ROI%"].map(h=><th key={h} style={{textAlign:"left",padding:"3px 4px",color:P.ash,fontSize:8,borderBottom:`1px solid ${P.border}`,whiteSpace:"nowrap"}}>{h}</th>)}</tr></thead>
            <tbody>{engine.positions.map(p=>{
              const lp=engine.bots[p.sym]?.lastPrice||p.ep;
        ...
        {/* Closed Trades */}
        {engine?.trades?.length>0&&<div style={{marginBottom:6}}>
          <div style={{fontFamily:Fm,fontSize:9,color:P.violet,letterSpacing:"0.1em",marginBottom:3,fontWeight:600}}>KAPANMIŞ ({engine.trades.length})</div>
          <div style={{overflowX:"auto",maxHeight:70,overflowY:"auto"}}><table style={{width:"100%",borderCollapse:"collapse",fontFamily:Fm,fontSize:9}}>
            <thead><tr>{["Coin","Yön","Kaldıraç","Giriş","Çıkış","Hacim","Sonuç","K/Z","ROI%"].map(h=><th key={h} style={{textAlign:"left",padding:"2px 4px",color:P.ash,fontSize:8,borderBottom:`1px solid ${P.border}`,position:"sticky",top:0,background:P.card,whiteSpace:"nowrap"}}>{h}</th>)}</tr></thead>
            <tbody>{[...engine.trades].reverse().map((t,i)=><tr key={i} style={{borderBottom:`1px solid ${P.border}`}}>
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:3}}>
          ...
          <button onClick={stop} style={{fontFamily:Fs,fontSize:10,fontWeight:600,padding:"4px 14px",borderRadius:6,background:"rgba(239,68,68,0.1)",border:"1px solid rgba(239,68,68,0.25)",color:P.danger,cursor:"pointer"}}>⏹ Durdur</button>
        </div>
  ...
  return <div style={{minHeight:"100vh",background:P.ink,color:P.mist,fontFamily:Fs}}>
    ...
    <div style={{display:"flex",height:"calc(100vh - 56px)"}}>
      ...
      <div style={{width:250,minWidth:250,borderRight:`1px solid ${P.border}`,padding:"14px 12px",overflowY:"auto",background:P.card+"88"}}>
        ...
        <div style={{fontFamily:Fm,fontSize:10,color:P.ember,letterSpacing:"0.12em",marginBottom:6,textTransform:"uppercase",fontWeight:600}}>📡 CANLI SİNYAL</div>
        ...
        {sigOn&&sig&&<div style={{fontFamily:Fm,fontSize:10,color:sig.signalColor,fontWeight:600}}>{sig.signal}{sig.alerts.length>0&&<span style={{color:P.danger,animation:"pulse 0.8s infinite"}}> • {sig.alerts.filter(a=>a.hot).length} yakın!</span>}</div>}</>}
    ...
    {/* PAPER TRADING PANEL */}
    <PaperPanel engine={paperEng} setEngine={setPaperEng} syms={syms} cfg={cfg}/>
  </div>;
}
